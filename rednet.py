...
class RedNetResizeWrapper(nn.Module):
    ...
    def forward(self, rgb, depth):
        ...
        #print("rgb size in model ", rgb.shape)
        #print("depth size in model ", depth.shape)
        rgb = rgb.float() / 255
        #if self.resize:
        ...
        #with torch.no_grad():
        preds = self.rednet(rgb, depth)
        #print("grad fn preds ",preds.grad_fn)
...
def load_rednet(device, ckpt="", resize=True, stabilize=False):
    ckpt = os.path.join("ckeckpoint/", ckpt)
    ...
    if  ("_" in ckpt):
        ...
        for k in old_keys:
            new_k = k.split("rednet.")[1]
            state_dict_new[new_k] = state_dict[k]
            del state_dict_new[k]
        state_dict = {
            (k[len(prefix):] if k[:len(prefix)] == prefix else k): v for k, v in state_dict_new.items()
        }
